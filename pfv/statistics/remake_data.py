...
def  remake_data(mac,st,ed,flow,stay,flow_cnt,stay_cnt,floor):
    ...
    for flow_info in flow_ap_list[:]:
        ...
        # if len(stay_ap_list) == 0:
        #     pass
        # elif flow_info["start"] > stay_ap_list[0]["start"]:
        #     pass

        # print(list(map(key_id,flow_info["route"])))
        # 不要情報の除去
        ...
        for ap in flow_info["route"]:
            if len(route) == 0:
                ...
                route.append(ap)
            # elif back_count != 0:
            #     if (route[-2]["datetime"] - route[-2 - back_count*2]["datetime"]).seconds >= INTERVAL*(1+back_count):
            #         # if (del_list != []) and (del_list[0]["datetime"] > route[-2]["datetime"]):
            #         #     route = route + del_list[0:]
            #         #     del_list = []
            #         # else:
            #         pass
            #     else:
