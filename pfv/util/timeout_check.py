...
def save_rttmp(ip,floor,pcwl_id):
	...
	try:
		...
		st_to = time.time()
		with urllib.request.urlopen(url=LOGIN_URL, data=b'cmd=/usr/sbin/station_list%20-i%20ath0', timeout=3.0) as page:
		    for line in page.readlines():
		        html.append(line.decode('utf-8'))
		ed_to = time.time()

		new_data = {"ip":ip,"get_time_no":now,"floor":floor,"pcwl_id":pcwl_id,"difference":rounding(ed_to-st_to,2)}
		...
		# for line in html:
			# if dbsave_flag:
				...
				# if time_stamp > ts_th:
					# add : floor, pcwl_id
					# new_data = {"ip":ip,"get_time_no":now,"mac":mac, "rssi":rssi,"dbm":rssi - 95,"floor":floor,"pcwl_id":pcwl_id,"difference":ed_to-st_to}
		...
		to_data = {"datetime":now, "timeout_ip":ip, "floor":floor, "pcwl_id":pcwl_id, "TO_type":"Normal timeout","difference":rounding(ed_to-st_to,2)}
		...
		sto_data = {"datetime":now, "timeout_ip":ip, "floor":floor, "pcwl_id":pcwl_id, "TO_type":"Socket timeout","difference":rounding(ed_to-st_to,2)}
...
def save_function(pcwlip): #pcwlip: type:dict, elements: ip, floor, pcwl_id
	if (pcwlip["ip"] != "10.0.11.55"):
		...
		if len(data_list["data_list"]) >= 1:
			...
			db.to_check.insert(data_list["data_list"])
		if len(data_list["to_list"]) >= 1:
			db.to_check_err.insert(data_list["to_list"])
...
@timeout_decorator.timeout(27.0) # cannot use on Windows
