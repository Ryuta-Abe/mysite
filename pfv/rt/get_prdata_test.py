...
def save_function(pcwlip): #pcwlip: type:dict, elements: ip, floor, pcwl_id
	"""
    store RSSI data of given PCWL into several DB shown below
    @param  pcwlip : dict : {"ip", "floor", "pcwl_id"}
	rttmp(real time tmp. col.) : dict(every received data) : {"ip", "get_time_no", "mac", "rssi", "dbm"}
	trtmp(tag rttmp. col.) : data whose tag is in tag_list
	timeoutlog : statistics of timeout when timeout occurs 
	...
	if (pcwlip["ip"] != "10.0.11.55"):
		...
		for data in data_list["data_list"]:
			if data["mac"] in tag_list:  # store data whose tag is in tag_list into trtmp
