...
dataset = []
dataset.append({"st_id":11,"st_dt":datetime.datetime(2014,11,10,11,10,9),"ed_id":1,"ed_dt":datetime.datetime(2014,11,10,11,10,59)})
dataset.append({"st_id":1,"st_dt":datetime.datetime(2014,11,10,11,10,9),"ed_id":5,"ed_dt":datetime.datetime(2014,11,10,11,10,59)})
dataset.append({"st_id":2,"st_dt":datetime.datetime(2014,11,10,11,10,59),"ed_id":4,"ed_dt":datetime.datetime(2014,11,10,11,11,9)})
dataset.append({"st_id":4,"st_dt":datetime.datetime(2014,11,10,11,11,9),"ed_id":16,"ed_dt":datetime.datetime(2014,11,10,11,11,19)})

for data in dataset:
	ab_time = data["ed_dt"] - data["st_dt"]
	interval = int(ab_time.total_seconds())
	tmp_plist = []
	tmp_plist += pfvinfo.objects(datetime = data["st_dt"]).scalar("plist")
	if len(tmp_plist) == 0: # この時間の情報がまだDBに登録されていない場合
		make_empty_pfvinfo(data["st_dt"]) # この時間の空の情報を作成
		tmp_plist += pfvinfo.objects(datetime = data["st_dt"]).scalar("plist")
	tmp_plist = tmp_plist[0]
	route_info = [] # 経路情報の取り出し
	route_info += pcwlroute.objects(query__all = [data["st_id"], data["ed_id"]])
	route_info = optimize_direction(data["st_id"],data["ed_id"],route_info[0]["dlist"])
	print("[出発点,到着点] = "+str([data["st_id"], data["ed_id"]])+" , 間隔 = "+str(interval)+" 秒")
	...
	if interval == 10:
		for route in route_info: # routeの例：[{'direction': [2, 3], 'distance': 75.16648189186454}, {'direction': [3, 4], 'distance': 69.6419413859206}]
			for node in route:
				...
				pfvinfo.objects(datetime = data["st_dt"]).delete()
				save_pfvinfo(tmp_plist,data["st_dt"])
		print(str(data["st_dt"])+"のpfvinfoを登録完了, 経路分岐 = "+str(len(route_info)))

	# 20秒以上の間隔の場合
	else:
		num = int(interval / 10) # 40秒間隔の場合, num = 4
		tlist = [data["st_dt"], # tlistは時間情報のDBから取り出すように後で変える
		...
		for route in route_info: # ある経路に対して以下を実行
			...
			while t_count < num:
				...
				if tmp_distance >= (total_distance*t_count/num): # 一時累計距離がしきい値を超えたら以下を実行
					...
					n_count += 1
			st_ed_info.append({"st":st_ed_info[-1]["ed"],"ed":data["ed_id"]}) # st_ed_infoの完成,例：[{'ed': 2, 'st': 1}, {'ed': 3, 'st': 2}]
...
# # 保存テスト
# plist = []
# plist.append({"direction":[1,2],"size":3})
# plist.append({"direction":[2,3],"size":2})
# plist.append({"direction":[3,4],"size":5})
# d = datetime.datetime.today()
# save_pfvinfo(plist,d)
